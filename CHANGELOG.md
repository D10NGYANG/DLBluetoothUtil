...
`1.0.1` 是日志精简补丁版本，不改变公共 BLE API、连接状态机或通讯行为。

### 日志精简

- Android 每条扫描广播继续输出 `[scan.result]`，但移除已由扫描生命周期表达的 `type`、固定为
  `ALL_MATCHES` 的 `callbackType` 以及可由时间戳计算的 `elapsedMs`；普通结果仅保留设备地址、名称和 RSSI。
- JS、WasmJS 和 iOS 同步移除扫描结果中的重复扫描类型；Android 批量结果仅额外保留 `source=batch`。
- Android/iOS 正常操作不再逐步输出入队、恢复就绪、开始、提交、Runner 接收和完成事件；超时、取消、
  异常、进入恢复及恢复完成日志保持不变。
- 移除连接、服务发现、特征写入、通知配置和 MTU 的原生成功回调日志；最终成功/失败、原生错误详情和
  公共连接生命周期日志仍然保留。
- BLE 收发 `[ble.tx]`、`[ble.rx]` 的完整设备、特征和 HEX payload 格式不变。一次成功写入从通常 9 条
  日志缩减为 `[ble.tx]` 与 `[write.success]` 两条。

完整日志兼容说明和升级检查见 [1.0.1 发布说明](docs/RELEASE_1.0.1.md)。
