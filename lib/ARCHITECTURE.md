...
- 关键生命周期日志统一使用可检索的事件名和 `key=value` 字段；正常路径记录公共入口和最终结果，
  移动端调度细节只在超时、取消、异常和恢复时输出；
- 移动端操作使用内部 `operationId` 关联通讯内容和最终结果或异常，避免逐次记录入队、加锁、提交和
  Runner 接收等内部经过点；
- 每条扫描广播只记录地址、名称和 RSSI；扫描类型记录在开始和结束事件中，不在每条结果中重复；
